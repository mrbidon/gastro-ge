...
//	console.log(fields);
	...
	for(var i = 0; i < fields.length; i++){
		...
		if((field[0] == '"') &&(field[field.length-1] == '"') ){
			results.push(field.substr(1,field.length-1))
		}else{
			results.push(field)
		}
...
geguide.GeGuide = function(html_id){
    ...
    this.map.setCenter(
        new OpenLayers.LonLat(3.999, 49.596).transform(
	...
	$.ajax({
		...
		success: function(data){
			...
			gastro_registry = geguide.CSVToArray(data);
			...
			console.log(gastro_registry);
			for(var i = 1; i < gastro_registry.length; i++){
				line = gastro_registry[i];
				var adresse = 	{
												 "id":  parseInt(line[0]),
												 "titre": line[1],
												 "detail": line[2],
